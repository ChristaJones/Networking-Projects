...
char * LRU(vector<CacheLine> &c, int servsd, sockaddr_in serv, char * servhost, socklen_t serv_size, int msize, string subject, char *g, string ip, int * msg2size)
{
	...
	auto it = std::find_if(c.begin(), c.end(), [q](const cache_line& e) {return e.query == q;});
	if(it != end())/*cache hit*/
	{
		memcpy(r, (*it).resonse.c_str(), s.length());
		(*it).time = clock();		
	}
	else/*cache miss*/
	{
				...
				if (FD_ISSET(frsock, &copy)) {
		         ...
		         if (!get) {
		                 // disconnect from client 
							  ...
							  cout<<"Client "<<clientsd<<" has disconnected\n";
		                 ...
		                 close(frsock);
							...
							int msg2size;
							 inet_ntop(AF_INET, &(client.sin_addr), ip_addr, 20);
								...
								response = LRU(cache, serversd, server, serverhost, server_size, msgsize, getname, get, ip);
